...
#pragma NDIS_INIT_FUNCTION(jcNDISRegisterDevice)
    ...
    )
{
    ...
    DEBUGP(DL_TRACE, "==>jcNDISRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = jcNDISDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = jcNDISDispatch;
    DispatchTable[IRP_MJ_CLOSE] = jcNDISDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = jcNDISDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==jcNDISRegisterDevice: %x\n", Status);
