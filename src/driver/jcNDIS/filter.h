...
#define FILTER_FRIENDLY_NAME        L"jcNDIS NDIS LightWeight Filter"
#define FILTER_UNIQUE_NAME          L"{3f5bde0c-8154-4528-927f-9b04325a236d}" //unique name, quid name
#define FILTER_SERVICE_NAME         L"jcNDIS"

//
// The filter needs to handle IOCTLs
//
#define LINKNAME_STRING             L"\\DosDevices\\jcNDIS"
#define NTDEVICE_STRING             L"\\Device\\jcNDIS"
...
NDIS_STATUS
jcNDISRegisterDevice(
    VOID
    );

_IRQL_requires_max_(PASSIVE_LEVEL)
VOID
jcNDISDeregisterDevice(
    VOID
    );

DRIVER_DISPATCH jcNDISDispatch;

DRIVER_DISPATCH jcNDISDeviceIoControl;
